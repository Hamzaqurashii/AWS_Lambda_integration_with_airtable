...
// src/functions/deleteRecord/handler.ts
__export(exports, {
  main: () => main
});
...
var hello = async (event) => {
  var base = new import_airtable.default({ apiKey: event.body.apiKey }).base(event.body.baseID);
  base("Table 2").destroy(event.body.fields, function(err, deletedRecords) {
    if (err) {
      console.error(err);
      return;
    }
